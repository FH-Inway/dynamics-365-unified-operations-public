...
description: Learn about the basic concepts of the point of sale (POS) extension in Microsoft Dynamics 365 Commerce.
author: josaw1
ms.date: 02/25/2026
ms.topic: how-to
...
|---|---|---|
| Extension point | A specific location where POS can be extended, or a technique that you can use to add new functionality to POS. | <p>**PreOperationTrigger** invocation point</p><p>Addition of a new view</p> |
...
| Extension package | A set of extensions that, when combined, enable a custom end-to-end POS scenario. | A package that contains an operation that opens a new view |

## Types of extensions

POS extensions can be classified into two types:

- *Extend* extensions modify POS functionality by changing existing pages or workflows.
...
The POS API library contains a set of modules that are organized based on the way that the extension uses them.

- **Extend**

    - These modules help extensions extend POS components and workflows.
    - Subdirectories are organized first by the type of component that the extension extends and then by the feature area that the extension extends.
...
- **Create**
    ...
    - Subdirectories are organized by the type of component that the extension creates.
...
Each POS extension contains one or more TypeScript modules. Extension classes must inherit from the applicable base class that the POS API exposes. You can import the components and types in the POS API into the extension module by using a non-relative import of a POS API module, as shown in the following example.
...
The fields in **ExtensionPackageDefinition** must match the values that you used when you created the POS extension package.
...
- **Publisher** – The publisher must match the publisher that you specify in the extension package's **manifest.json** file.
