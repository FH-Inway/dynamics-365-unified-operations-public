...
- When you turn on the **Group view** toggle button on the **Model mapping designer** or **Format designer** page, the configured data sources are grouped by the data source type.

    :::image type="content" source="./media/er-data-sources-toggle-group-view2.png" alt-text="Screenshot of data sources grouped by type on the Format designer page.":::

- When you turn off the **Group view** toggle button, the data sources are sorted in alphabetical order.

    :::image type="content" source="./media/er-data-sources-toggle-group-view1.png" alt-text="Screenshot of data sources in alphabetical order on the Format designer page.":::
...
- Select **Show mapped only** to list only the data sources that you explicitly use in configured bindings.
- Select **Show not-mapped only** to list only the data sources that you don't use in configured bindings.

:::image type="content" source="./media/er-data-sources-select-mapped-only-view.png" alt-text="Screenshot of Format designer page in Show mapped only view mode.":::

### Model mapping designer

To add a new data source, select the data source type in the **Data source types** pane on the left side of the **Model mapping designer** page. This pane shows the tree of supported ER data sources.

The **Data sources** pane in the middle of the page shows the data sources that you already added to the current model mapping. Use the **Add root** and **Add** buttons to add a new data source entry.
...
:::image type="content" source="./media/er-data-sources-new-child-ds-entry.gif" alt-text="Screenshot of adding a new child data source on the Model mapping designer page.":::
...
>
> :::image type="content" source="./media/er-data-sources-hide-ds-pane.gif" alt-text="Screenshot of collapsing the Data source types pane on the Model mapping designer page.":::

### Format designer

The **Mapping** tab of the **Format designer** page shows the data sources that you already added to the current format. Use the **Add root** and **Add** buttons to add a new data source entry.
...
:::image type="content" source="./media/er-data-sources-new-root-ds-entry.gif" alt-text="Screenshot of adding a new top-level data source on the Format designer page.":::

> [!NOTE]
> The following data source types are available for selection only when you turn on the **Show details** toggle button on the **Format designer** page:
>
> - Class
> - Object
> - Table
...
You can specify the properties described in this section for any data source type.

### Name property

Because data source names must be unique in an ER component, you must provide the **Name** property. Use this property to uniquely identify the added data source in the configured ER component.

### Label property

The **Label** property is optional. Use it to specify an alternative name for the data source. To specify language-dependent text for the label of the editable data source, select **Translate**. When you [translate](er-design-multilingual-reports.md#entry) the label, the **Model mapping designer** or **Format designer** page presents it in the preferred language of the current user.
...
When you specify this property for a data source that lets you enter information in a data entry field, the specified label is the prompt text for that data entry field. When you translate the label, the prompt text for the data entry field appears in your preferred language, or the preferred language of the current user.

### Help property

The **Help** property is optional. Use it to specify a description of the data source. To specify language-dependent text for the description of the editable data source, select **Translate**. When you translate the description, the **Model mapping designer** or **Format designer** page presents it in the preferred language.

The following illustration shows the entry of data source properties on the **Format designer** page.

:::image type="content" source="./media/er-data-sources-ds-common-properties-entry.gif" alt-text="Screenshot of entering data source properties on the Format designer page.":::

## <a name="data-model-group"></a>Data model group

You can configure data sources of different types that represent a data model that you select as the basis for the editable ER model mapping or ER format component. This section provides a summary of the properties that are common to all types of ER data sources that belong to the **Data model** group.
...
> [!TIP]
> You can leave the **Definition** property blank if the default root definition is specified in the referenced data model.

### Version property

The optional **Version** property specifies the revision of the data model component for this data source. To review the list of data model revisions that are available in the current Finance instance, expand the **Versions** FastTab on the **Configurations** page for the appropriate data model configurations.
...
> [!TIP]
> This data source is automatically added for any ER format component that you create to generate an outbound document.
...
:::image type="content" source="./media/er-data-sources-ds-data-model-properties.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Data model type.":::

### <a name="model-enumeration"></a>Model enumeration

The **Data model enumeration** data source represents the enumeration of the base data model. You usually need this data source when you configure an ER [expression](er-formula-language.md) to compare the value of a data model field with a specific enumeration value.

The mandatory **Model enumeration** property points to a data model enumeration of the selected ER data model component. It specifies which values of the referenced ER data model enumeration are used in this data source.
...
:::image type="content" source="./media/er-data-sources-ds-data-model-enum-properties.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Data model enumeration type.":::

### <a name="model-enumeration-user-input-parameter"></a>Model enumeration user input parameter

The **Model enumeration user input parameter** data source represents the enumeration of the base data model. You usually need this data source when you want the value of the referenced enumeration that's specified in the related data entry field in the dialog box at runtime, before the ER format runs.

The mandatory **Model enumeration** property points to a data model enumeration of the selected ER data model component. It specifies what values of the referenced ER data model enumeration this data source returns.
...
Use the **Edit visibility** function to configure an ER expression that specifies, at runtime, whether the appropriate data entry field should be shown in the dialog box at runtime.
...
For more information, see [Use USER INPUT PARAMETER data sources to specify parameters for a report](er-user-input-parameter-data-sources.md).
...
Configure a data source of the *Model enumeration lookup* type to filter data in an ER format so that it's based on a set of abstract rules. You can then specify real rules outside the ER component designers by using the user interface (UI) that's automatically generated based on the settings of the **Lookup** data source of the corresponding ER format and the current legal entity data. When you run this ER format, the ER format's **Lookup** data source accesses the specified rules.

The mandatory **Model enumeration** property of the **Model enumeration lookup** data source points to a data model enumeration of the base ER data model component to specify what values of the referenced ER data model enumeration this data source returns.
...
The **Extended** property reveals more methods for accessing values of the configured data source.

For more information, see [Configure Lookup data sources to use ER application-specific parameters](er-lookup-data-sources.md).
...
> [!NOTE]
> You can only configure data sources of the **Format enumeration** group for an ER format component.

The mandatory **Enumeration** property references an ER format enumeration whose value the configured data source returns.
...
The following illustration shows a format enumeration that you configured for the editable format component.

:::image type="content" source="./media/er-data-sources-ds-configure-format-enum.gif" alt-text="Screenshot of configured format enumeration on the Format enumerations page.":::

You usually need this data source when you configure an ER expression to compare a specific format enumeration value with a value of the relevant enumeration that another data source returns.
...
:::image type="content" source="./media/er-data-sources-ds-format-enum-properties.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Format enumeration type.":::

### <a name="format-enumeration-user-input-parameter"></a>Format enumeration user input parameter

The **Format enumeration user input parameter** data source represents the enumeration of the editable format. You usually need this data source when you want to specify a value of the referenced enumeration in the related data entry field in the dialog box at runtime, before an ER format runs.

The mandatory **Format enumeration** property of the **Format enumeration user input parameter** data source points to an enumeration of the editable format to specify what values of the referred format enumeration this data source returns.

:::image type="content" source="./media/er-data-sources-ds-format-enum-uip-properties.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Format enumeration user input parameter type.":::
...
:::image type="content" source="./media/er-data-sources-ds-format-enum-expression.png" alt-text="Screenshot of configuration of an ER expression on the Format designer page to fill in an ER format element.":::
...
The mandatory **Format enumeration** property of the **Format enumeration lookup** data source points to a format enumeration of the editable format to specify what values of the referenced format enumeration this data source returns.

Set the **Cross-company** and **Extended** properties as described earlier for the *[Model enumeration lookup](#model-enumeration-lookup)* data source type.

## <a name="format-group"></a>Format group

### <a name="export-format"></a>Export format

Use the **Export format** data source to run another ER format during the execution of the editable format. The execution results of the nested ER format serve as a data source for the execution of the editable ER format.

Use the mandatory **Format** property to specify an ER format that must run during the execution of the editable format.

### <a name="import-format"></a>Import format

Use the **Import format** data source to parse an inbound business document. The results of the inbound document parsing can then be used in the format mapping to fill a data model.
...
:::image type="content" source="./media/er-data-sources-ds-import-format-structure.png" alt-text="Screenshot of format structure on the Format designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-import-format-mapping.gif" alt-text="Screenshot of data source of the Import format type on the Model mapping designer page.":::

For more information, see [Parse incoming documents](er-parse-incoming-documents.md).

## <a name="dynamics-365-finance-group"></a>Dynamics 365 Finance group

### <a name="class"></a>Class

Use the **[Class](er-formula-supported-data-types-composite.md#class)** data source to call a static method of a public class from your ER solution. You can dynamically define argument values for calling classes at runtime. To invoke a class, configure a data source of the *Calculated field* type with an expression that references the desired class of application source code. For more information, see [Design ER expressions to call application class methods](./tasks/design-expressions-app-class-er.md).

You can add a data source of the *Class* type in the editable **Model mapping** component. If you identify the editable model mapping as the preferred one for execution, set the **Integration point** option of the **Class** data source to **Yes**. For more information about this option, see [API to run a format mapping for the generation of outbound documents](er-apis-app10-0-11.md#api-to-run-a-format-mapping-for-the-generation-of-outbound-documents).

### <a name="object"></a>Object

Use the **[Object](er-formula-supported-data-types-composite.md#object)** data source to call a public method of a class instance from your ER solution. You can dynamically define argument values for calling classes at runtime. To invoke a class, configure a data source of the *Calculated field* type with an expression that references the desired class of application source code.

Add a data source of the *Object* type in the editable **Model mapping** component. If you identify the editable model mapping as the preferred one for execution at runtime, set the **Integration point** option of this **Object** data source to **Yes**.

### <a name="enumeration"></a>Enumeration

The **Enumeration** data source represents the application enumeration. You usually need this data source when you configure an ER expression to compare a specific application enumeration value with a value of the relevant enumeration that another data source returns.
...
:::image type="content" source="./media/er-data-sources-ds-app-enum-properties.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Application enumeration type.":::
...
Use the **Number sequence** data source to return a value from the referenced number sequence for the current application instance. When a data consumer calls this data source, the referenced number sequence increments and returns the received value [returned](#num-sequence-data-source-sample).

You must specify the mandatory **Number sequence code** property to identify the required number sequence.
...
Use the **Table** data source to call static methods of an application table, entity, or view. You call these static methods in the same way that you call static methods of an application class, as described earlier.

You must specify the mandatory **Table** property to identify the required table, entity, or view.

### <a name="table-records"></a>Table records

Use the **Table records** data source to fetch records from an application table, entity, or view.

You must specify the mandatory **Table** property to identify the required table, entity, or view.

If the table, entity, or view that you reference in the **Table** property keeps records company-specific, specify the **Cross-company** property.

- Set this property to **No** to fetch records only for the company that an ER solution runs in scope of.
- Set this property to **Yes** to fetch records for all companies of the current application instance.

Set the **Ask for query** option to **Yes** to enable the system query dialog box at runtime, so that users can filter data for the configured data source. For more information, see [Cross-company data sources in Electronic reporting (ER)](er-cross-company-data-sources.md).

If you identify the editable model mapping as the preferred one for execution, set the **Integration point** option to **Yes**.

Use the **Select fields** function to limit the list of fields whose values are fetched at runtime. For more information, see [Improve performance of ER solutions by reducing the number of table fields that are fetched at runtime](er-reduce-fetched-fields-number.md).
...
Use the **Empty container** data source as the container for other data sources. Use it to organize your data sources or to make groups of data sources of different types for specific purposes.
...
:::image type="content" source="./media/er-data-sources-ds-endless-list-format.png" alt-text="Screenshot of structure of the sample ER format on the Format designer page.":::
...
- The **Item** format element is bound to the `ListofItems` data source that returns three records. The **Maximal number of elements per file** property of this format element is configured to split generated output into multiple XML files, each of which must contain only one `Item` XML node.
    ...
    :::image type="content" source="./media/er-data-sources-ds-endless-list-bindings1.png" alt-text="Screenshot of binding of the Item format element in the sample ER format on the Format designer page.":::
...
- The **File parameters list** property of the **File** format element references the `Parms` data source of the *Endless list* type to ensure that this data source is invoked whenever the output split occurs. Because the `Parms` data source contains the nested <a name="num-sequence-data-source-sample"></a>`NumSequence` data source of the *[Number sequence](#number-sequence)* type, the application number sequence that's referenced in `NumSequence` is also iterated for each split. Therefore, the `Parms.NumSequence` ER expression that's configured in the **File name** property of the **File** component generates a unique name for each output file, based on the current value of the referenced number sequence.
    ...
    :::image type="content" source="./media/er-data-sources-ds-endless-list-bindings2.png" alt-text="Screenshot of Data source properties dialog box for a data source of the Number sequence type.":::

The following illustration shows the result of running the previously described sample ER format. The result represents a zip file that contains XML output that's split into three uniquely named XML files.

:::image type="content" source="./media/er-data-sources-ds-endless-list-output.png" alt-text="Screenshot of result of running the sample ER format on the Format designer page.":::

For more information, see [Split generated XML files based on file size and content quantity](er-split-files.md).

### <a name="user-input-parameter"></a>User input parameter

Use a data source of the *User input parameter* type to let users specify a value of the referenced application enumeration or application EDT in the related data entry field in the dialog box at runtime, before an ER format runs.

The mandatory **Operations data type name (EDT, enum)** property of the **User input parameter** data source points to an application enumeration or an application EDT to specify what values this data source returns.
...
> [!IMPORTANT]
> When you add a new **Barcode** data source, place it under another item (container) as a nested element. The **Barcode** data source appears in the configured ER component as a function that has the **Value** parameter of the *[String](er-formula-supported-data-types-primitive.md#string)* type. Provide the bar code text as the argument of this parameter.
...
The following illustration shows the **Calculated field** data source that sorts the initially provided list of records in alphabetic order.

:::image type="content" source="./media/er-data-sources-ds-calc-field-format1.gif" alt-text="Screenshot of configured data source of the Calculated field type on the Format designer page.":::

When you add a new **Calculated field** data source by placing it under another item as a nested element, you can specify multiple parameters that you can then reference in the configured ER expression. You can also configure other formulas to call this data source. In this way, you can control the values of arguments of those parameters. Therefore, you can reuse a single data source in many bindings and reduce the total number of data sources that you must configure in an ER model mapping or ER format component.

:::image type="content" source="./media/er-data-sources-ds-calc-field-format2.gif" alt-text="Screenshot of using a configured data source of the Calculated field type on the Format designer page.":::

The following illustration shows the **Calculated field** data source that sorts the initially provided list of records according to the sort direction that's specified at runtime.

:::image type="content" source="./media/er-data-sources-ds-calc-field-format3.png" alt-text="Screenshot of data source of the Calculated field type that's configured for sorting on the Format designer page.":::

The sort order of this data source is specified by using the value of the argument of the data source parameter.

:::image type="content" source="./media/er-data-sources-ds-calc-field-format4.png" alt-text="Screenshot of setup of the sort order for the data source of the Calculated field type on the Format designer page.":::

For more information, see [Support parameterized calls of ER data sources of the Calculated field type](er-calculated-field-type.md).

### <a name="data-collection"></a>Data collection

You can configure an ER model mapping or an ER format to process specific elements, such as records of a model mapping data source or format elements, in a specific order. Additionally, you can configure a data source of the *Data collection* type to collect, during element processing at runtime, values of data model fields or values of bindings of the processed format elements, and then sum those values. The data source then uses the collected values to fill in a generated document or perform other calculations.
...
When you set the **Collect all values** option to **Yes**, the **Sum(Flag)** parameterized property becomes available. Use this property to get the total amount of all currently collected values. In this property, the **Flag** argument is a *Boolean* value that indicates whether the total value must be reset.

- If the value *False* is provided, summation continues from the previously collected amount.
- If the value *True* is provided, a new summation starts.
...
:::image type="content" source="./media/er-data-sources-ds-data-collection-format.png" alt-text="Screenshot of configured data sources of the Data collection type on the Format designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-data-collection-result.png" alt-text="Screenshot of result of running the sample ER format that has data sources of the Data collection type on the Format designer page.":::

For more information, see [Use DATA COLLECTION data sources in Electronic reporting formats](er-data-collection-data-sources.md).

### <a name="financial-dimensions-details"></a>Financial dimensions details

When you configure a data source of the *Table record* type that references an application table that supports the relation to financial dimensions, you see that this ER data source offers a field of the *Financial dimensions* type for every financial dimension relation. This field exposes the **Main account and dimensions** record list that enables access to financial dimensions as the list of records. Every record in this list represents a single dimension.
...
When you set the **Ask for dimensions** option to **Yes**, the **Financial dimensions selection** option is enabled, and you can specify the scope of financial dimensions. The following options are available:

- Select **All** to use all financial dimensions of the current application instance.
- Select **Legal entity** to use only the financial dimensions of the company that an ER solution runs in the scope of.
...
Set the **Ask for main account** option to **Yes** to let the user select the main account as part of the list of dimensions in the dialog box at runtime. If you set the option to **No**, the main account isn't included in the list of dimensions, and the **Is main account mandatory** option is enabled. If you set the **Is main account mandatory** option to **Yes**, the main account is included in the list of dimensions, regardless of the user's selection.

The following illustration shows the data source of the *Financial dimensions details* type that you can use to specify the scope of financial dimensions for fields of the *Financial dimensions* type when the configured model mapping is used at runtime.

:::image type="content" source="./media/er-data-sources-ds-fin-dim-mapping2.png" alt-text="Screenshot of the configured data source of the financial dimensions details option being highlighted.":::
...
When you configure a data source of the *Table record* type that references an application table that supports the relation to inventory dimensions, you see that this ER data source offers a field of the *Inventory dimensions* type for every inventory dimension relation. This field exposes the **InventDim** record list that enables access to inventory dimensions as the list of records. Every record in this list represents a single dimension.
...
:::image type="content" source="./media/er-data-sources-ds-inv-dim-mapping1.png" alt-text="Screenshot of configured data source of the Table record type on the Model mapping designer page.":::

Use the **Inventory dimensions details** data source in the configured ER component to specify how the scope of inventory dimensions is defined for any report that uses this component when a field of the *Inventory dimensions* type is accessed by a data consumer. Set the **Using dimensions** property to specify the scope of inventory dimensions. The following options are available:
...
The following illustration shows the data source of the *Inventory dimensions details* type that you can use to specify the scope of inventory dimensions for the **Inventory dimensions* fields when the configured model mapping is used at runtime.

:::image type="content" source="./media/er-data-sources-ds-inv-dim-mapping2.png" alt-text="Screenshot of configured data source of the Inventory dimensions details type on the Model mapping designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-groupby-setting1.png" alt-text="Screenshot of configured data source of the Group by type for grouping records by code value on the Edit Group By parameters page.":::
...
:::image type="content" source="./media/er-data-sources-ds-groupby-setting2.png" alt-text="Screenshot of configured data source of the Group by type for calculating the total amount on the Edit Group By parameters page.":::

Follow these steps to configure grouping parameters on the **Edit Group By parameters** page.

- To specify the base data source for grouping, select the desired data source on the right side of the page, then select **Add field** \> **What to group**.
- To specify a field for grouping, select the desired field of the specified base data source on the right side of the page, then select **Add field to** \> **Grouped fields**.
- To specify a field for calculating the aggregated value, select the desired field of the specified base data source on the right side of the page, then select **Add field to** \> **Aggregation fields**.
- Set the **Source list is ordered by group key** option to **Yes** to sort the group of records based on the values of fields that you select for grouping.
...
:::image type="content" source="./media/er-data-sources-ds-groupby-format1.png" alt-text="Screenshot of configured data sources of the Group by type on the Format designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-groupby-format2.png" alt-text="Screenshot of result of running the sample ER format that has data sources of the Group by type on the Format designer page.":::

For more information, see [Group records and aggregate calculations by using GROUPBY data sources](er-groupby-data-sources.md).

## <a name="join"></a>Join

You can configure the **Join** data source to fetch records from several base data sources and return a joined list of records that contains fields from the records of the base data sources.

> [!TIP]
> Because you can join many data sources that have multiple fields, use the **Select fields** function for any base data source of the configured **Join** data source to limit the list of fields whose values are fetched at runtime. For more information, see [Improve performance of ER solutions by reducing the number of table fields that are fetched at runtime](er-reduce-fetched-fields-number.md).
...
:::image type="content" source="./media/er-data-sources-ds-join-format1.png" alt-text="Screenshot of configured data sources on the Format mapping designer page.":::

The following illustration shows the configuration of a data source of the *Join* type on the **Join designer** page. To add a data source to the joined list, select the data source on the right side of the page, then use the **Add** button above the **Joined lists** grid.

:::image type="content" source="./media/er-data-sources-ds-join-setting.gif" alt-text="Screenshot of configured data source of the Join type on the Join designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-join-format2.png" alt-text="Screenshot of configured data sources on the Format designer page.":::
...
:::image type="content" source="./media/er-data-sources-ds-join-format3.png" alt-text="Screenshot of result of running the sample ER format that has a data source of the Join type on the Format designer page.":::

For more information, see [Use JOIN data sources to get data from multiple application tables in Electronic reporting (ER) model mappings](er-join-data-sources.md).

## <a name="table-metadata"></a>Table metadata

You can configure a **Table metadata** data source to simultaneously access the metadata and data of one or many ER data sources that you configured to return data of the *Record list* type.
...
:::image type="content" source="./media/er-data-sources-metadata-ds-entry.png" alt-text="Screenshot of Editor button in the Data source properties dialog box.":::

You can add the following items to the editable **Table metadata** data source:

- A data source that returns *Record list* data. Select the desired data source on the right side of the **Table metadata** page, then select **Add**.
- A field of the added data source. Select the desired field on the right side of the **Table metadata** page, then select **Add**.

:::image type="content" source="./media/er-data-sources-metadata-ds-configure.gif" alt-text="Screenshot of configuring a data source of the Table metadata type on the Table metadata page.":::

The following illustration shows the configured data source of the *Table metadata* type.

:::image type="content" source="./media/er-data-sources-metadata-ds-review.png" alt-text="Screenshot of preview the configured data source of the Table metadata type on the Model mapping designer page.":::
...
- **Fields** returns the list of fields of the current data source.
  ...
  - **Fields.Precision** returns the precision of the appropriate field if this field returns a numeric value.
    ...
    :::image type="content" source="./media/er-data-sources-metadata-ds-view-metadata.png" alt-text="Screenshot of metadata of the data source of the Table metadata type on the Debug Datasources page.":::
