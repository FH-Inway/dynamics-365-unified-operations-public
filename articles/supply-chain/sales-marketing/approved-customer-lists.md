...
Then use the buttons and fields described in the following subsections.
...
- **Customer code** – Select or review the customer relationship code that the item is approved for. The following values are available:
...
1. On the Action Pane, on the **Sell** tab, in the **Approved customer** group, select one of the following buttons:
    ...
    - **Setup** – Open a view of the **Approved customer list** page that is filtered by the customer account. When you create a line item in this context, you only have to specify approved items or item groups. The customer account is fixed to the account that you opened the list from.
    - **Approved customers** – Open the **Approved customer list by item** page, which shows the customers that are approved for the item. The list on this page resembles the list on the **Approved customer list** page, but it shows the customer account numbers of all customers that are defined in approved customer groups. Use the **Effective** and **Expiration** fields to filter the list based on a specific effective date or expiration date.
...
When you enter effective and expiration dates for an approved customer list record, validation is done only if the ACL check method is set to *Warning only* or *Not allowed*. (It doesn't validate them if the method is set to *No check*.) The dates are validated for the different documents, based on the setting of the **Date type** field on the **Sales and marketing parameters** page. This field specifies which date on the document is validated against the approved customer list record. The following values are available:
